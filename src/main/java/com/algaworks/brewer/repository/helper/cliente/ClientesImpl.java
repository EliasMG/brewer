	...
	private void adicionarFiltro(ClienteFilter clienteFilter, Criteria criteria) {
		if (clienteFilter != null) {
			...
			if (!StringUtils.isEmpty(clienteFilter.getCpfOuCnpj())) {
				criteria.add(Restrictions.ilike("cpfOuCnpj", clienteFilter.getCpfOuCnpj()));
			}
		}
	}
	
	private Long total(ClienteFilter clienteFiltro) {
		Criteria criteria = manager.unwrap(Session.class).createCriteria(Cliente.class);
		adicionarFiltro(clienteFiltro, criteria);
